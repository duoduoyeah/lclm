...
from nanochat.ops.attention import flash_attn, compute_window_sizes
...
class GPTConfig:
    ...
    # Rotary embedding variant. "rope" = standard RoPE; rope3d_* = 3D-RPE variants
    # (see nanochat/ops/rope_3d.py). chunk_size / base_chunk / theta_phi are only
    # consumed by the rope3d_* variants.
    rope_variant: str = "rope"
    rope_chunk_size: int = 128
    rope_base_chunk: Optional[float] = None  # rope3d_paper / rope3d_paper_rev; None → fall back to RoPE base
...
class GPT(nn.Module):
    ...
    def __init__(self, config, pad_vocab_size_to=64):
        ...
        self.window_sizes = compute_window_sizes(config.sequence_len, config.n_layer, config.window_pattern)
